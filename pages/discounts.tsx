...
import { Product, useAccountQuery, usePromotionsQuery } from 'typing';
...
const DiscountsPage = () => {
    ...
    const { data: promoData, loading: promoloading } = usePromotionsQuery();
    ...
    let correctedPromoData: { promotions: Product[] } = { promotions: [] }

    if (promoData !== undefined && promoloading === false)
        correctedPromoData.promotions = promoData.promotions.map((item) => {
            let tmpProduct = Object.assign({}, item);
            tmpProduct.itemQuantity = 1;
            return tmpProduct
        })
    let discountArrayFiltered: Product[] = [];

    if (data !== undefined && promoData !== undefined && promoloading === false)
